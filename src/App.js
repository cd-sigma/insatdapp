...
class App extends React.Component{
  ...
  async componentDidMount(){
    ...
    let proxy = await indexerContract.methods
      .accountAddr(indexCall[0])
      .call();
    this.setState({
      address:accounts[0],
      proxy:proxy
  ...
  async handleDeposit(amount,address,proxy){
    ...
    const spellData = await web3.eth.abi.encodeFunctionCall(abi[0], [
    ...
    const castData = await web3.eth.abi.encodeFunctionCall(abi[1], [
      ["AaveV2-v1.2"],
      [spellData],
      address,
    ]);
...
this.setState({message:"transaction going through..."})
    ...
    this.setState({
      message:`Succesfull Transaction! Transaction Hash :${txHash}! Party is pending😉`
    })
  ...
  render(){
    return(
      ...
      <button onClick={() => this.handleDeposit(this.state.amount,this.state.address,this.state.proxy)}>Deposit</button>
